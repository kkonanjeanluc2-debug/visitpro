...
export default function BottomNav({ utilisateur, notifCount = 0 }: BottomNavProps) {
  ...
  const dashboardItems: NavItem[] = [
    { href: '/dashboard', label: 'Dashboard', icon: GridIcon, badge: notifCount },
    { href: '/dashboard/mes-visites', label: 'Mes visites', icon: UserIcon },
  ...
  const items = ['secretaire', 'admin'].includes(role) ? secretaireItems : dashboardItems

  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 flex z-30 safe-area-pb">
      {items.map((item) => {
        const isActive = pathname === item.href || (item.href.length > 1 && pathname.startsWith(item.href) && item.href !== '/secretaire' && item.href !== '/dashboard')
